...
'''
Note: this is from github here: https://github.com/tanumoydhar/1D-Swift-Hohenberg-equation
'''
###########################################
...
kx = zeros(N_fm);
kx[0:int(N_fm / 2)] = arange(0, int(N_fm / 2));
kx[int(N_fm / 2 + 1):] = arange(int(-N_fm / 2 + 1), 0, 1);
...
L = (
            bf_c + 2.0 * alpha * alpha - alpha * alpha * alpha * alpha);  # linear operator -D^2 - D^4 in Fourier space, diagonal matrix!

Nsave = Nt / nsave + 1;  # number of saved time steps, including t=0
t = zeros(int(Nsave));
t[0:int(Nsave)] = arange(0, int(Nsave)) * (dt * nsave);  # t timesteps

U = zeros((int(Nsave), N_fm));  # matrix of (ti, xj) values.
...
for n in range(1, Nt):
    N0 = N1;
    N1 = b_2 * fft((real(ifft(v))) * (real(ifft(v)))) + (-1.0) * fft(
        (real(ifft(v))) * (real(ifft(v))) * (real(ifft(v))));
...
plt.tight_layout()
plt.show()
fig1.savefig('figs/spacetime_plot.pdf')
################################################################